/*
Firmware for : Cloud Locker/Distribution Board
Property of  : Kiny.io
...
//GLOBAL VARIABLES
String response,response1, query, forwardQuery,Box = "" , ID = "";
   ...
   if(in_serial.available()){
    ...
    if(query.length()>9){
      ...
      for(int i =0 ;i <= noOfBoxes ;i++){
        BoxNumbers[i] = query.substring(index,index+2).toInt() - 1 ;
        index = index + 3;
      }
      ...
      BoxNumber = query.substring(5,7).toInt() - 1;
      noOfBoxes = 1;
      //in_serial.print("BoxNumber = ");
      //in_serial.println(BoxNumber);
      Box = query.substring(5,7);
    }
    myCommand = false;
...
//    if(query.length()>3){
//      entryType = query.charAt(3);      // Check-In (T) OR Check-Out (R)
//    }
    switch(commandType)
    {
          ...
          case 'B': 
                    if(Box!="")
                    {
                      ...
                      if(noOfBoxes > 1){
                        ...
                        for(int s = 0 ; s < noOfBoxes; s++){
                          ...
                          BoxNumber = BoxNumbers[s] - (15 * slaveNumber);
                          if(BoxNumber<10)
                            response += 0;
                            
                          response += BoxNumber+1;
                    ...
                    else
                    {
                        ...
                        /////////Snippet Only for 30 or more lockers config (2 or more Distribution Boards Config)////////
                        out_serial.println("001:Q");
                        while(out_serial.available()==0);
                        while(out_serial.available()){
                          char z = out_serial.read();
                          response1 += z;
                          delay(3);
                        }
                        response1.trim();
                        ...
                        //in_serial.println("01:B01F&02F&03F&04F&05F&06E&07F&08F&09F&10F&11F&12F&13F&14F&15F&16F&17F&18F&19F&20E&21E&22F&23E&24F&25E&26E&27E&28E&29E&30E");                            
                        break;
                    }
                    break;
          ...
          //////////////////////////////////////Daisy chained Query (B command from previous slave) (starts here)//////////////////////////////////
          case 'Q':
                      response = '&';
                      firstIterationX = true;
                      for(int g =0; g <= 14; g++){
  
                            if(firstIterationX){
                              DoNothing = 1;
                              firstIterationX = false;
                            }
                            else
                            response += '&';
                      ...
                      in_serial.print(response);
                    
                    break;
          /////////////////////////////////////////////////////Daisy chained Query (Ends Here)//////////////////////////////////////////////////////           
          default:  break;
    }
