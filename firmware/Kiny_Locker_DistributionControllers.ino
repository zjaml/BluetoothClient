/*
Firmware for : Cloud Locker/Distribution Board/30 Locker
Property of  : Kiny.io
  ...
  if(in_serial.available()){
    ...
    case 'B': if(Box!=""){  // for B command with a BoxNumber
                ...
                if(noOfBoxes > 1){
                  ...
                  myNOB = noOfBoxes;
                  if(slaveNumber < 1){
                    for(int m = 0; m< noOfBoxes; m++){
                      if(BoxNumbers[m] > 15) myNOB--;
                    }
                  }
                  extraNOB = noOfBoxes - myNOB;

                  for(int s = 0 ; s < myNOB; s++){
                  ...
                  if(extraNOB){
                    commandNext = "";
                    commandNext = slaveNumber + 1;
                    commandNext += ID;
                    commandNext += ":X";
                    firstIteration1 = true;
                    for(int w = 0; w < extraNOB ; w++){
                      if(firstIteration1) firstIteration1 = false;
                      else commandNext += '&';
                      commandNext += BoxNumbers[myNOB+w];
                    }
                    out_serial.println(commandNext);
                    COUNTER = 0;
                    NoResponse = false;
                    response1 = "";
                    while(out_serial.available()==0){   // waiting for a response from next slave
    ...
    //////////////////////////////////////Daisy chained-Multi Box Query (B command from previous slave) (starts here)/////////////////////////
    case 'X':   response = "";
                response = '&';
                firstIterationX = true;
                for(int t = 0 ; t < noOfBoxes; t++){
                if(firstIterationX) firstIterationX = false;
                    else response += '&';
                    BoxNumber = BoxNumbers[t] - (15 * slaveNumber) - 1;
                    response += BoxNumbers[t];
                ...
                in_serial.print(response);
              break;
    /////////////////////////////////////////////////////Daisy chained-MultiBox Query (Ends Here)////////////////////////////////////////////////           
